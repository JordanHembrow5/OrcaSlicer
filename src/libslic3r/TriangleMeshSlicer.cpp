    ...
    std::function<void()>             throw_on_cancel)
{
    std::vector<Polygons> layers_p;
    {
        ...
        if(params.tolerance != MeshSlicingParamsEx::SlicingTol::Default && zs.size() > 1)
        {
            ...
            upper_z[0] = 2*zs[0];
            lower_z[0] = 0.01; // Don't set this too low or we can see incorrect slicing behaviour 
            float delta = 0.0;
            for(size_t i = 1; i < layer_n - 1; ++i)
            {
                delta = (zs[i+1] - zs[i-1]) / 4.0; // half the current layer height
                upper_z[i] = zs[i] + delta;
                lower_z[i] = zs[i] - delta;
            }
            delta = (zs[layer_n - 1] - zs[layer_n - 2]) / 2.0;
            upper_z[layer_n - 1] = zs[layer_n - 1]; // ensure we don't pop out the top of the model
